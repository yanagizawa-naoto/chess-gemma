"""Run N parallel Quick vs Thinker chess matches with balanced color assignment.
...
from chess_game import GameState, board_to_ascii
...
def build_user_msg(board: chess.Board, legal_uci):
    color = "白(W)" if board.turn == chess.WHITE else "黒(B)"
    legal_str = ", ".join(legal_uci)
    return (
        f"あなたは {color} の手番です。\n\n"
        f"現在の盤面（大文字=白, 小文字=黒）:\n{board_to_ascii(board)}\n\n"
        f"合法手 ({len(legal_uci)}): {legal_str}\n\n"
...
def extract_attempted(text):
    try:
        ...
        m = re.search(r"[a-h][1-8][a-h][1-8][qrbn]?", text)
        return m.group(0) if m else "(parse-failed)"


async def call_model(client, style, board, legal_uci, stats, max_retries=2):
    cfg = CFG[style]
    user_msg = build_user_msg(board, legal_uci)
    messages = [
    ...
    for attempt in range(max_retries + 1):
        ...
        messages.append({
            ...
            "content": (
                ...
                f"合法手は次のいずれかだけです: {', '.join(legal_uci)}。"
                f"必ずこの中から1つ選び、同じ JSON フォーマットで再回答してください。"
            ),
        })
    stats["fallbacks"] += 1
    stats["retries"] += max_retries
    return legal_uci[0], max_retries, True
        ...
        try:
            while not g.is_over:
                ...
                if not legal:
                    break
                style = side[g.turn]
                mv, retries, fb = await call_model(client, style, g.board, legal, stats)
                g.play(mv, retries=retries, forced_fallback=fb)
            w = g.winner()
            if w == "draw":
                winner_style = DRAW
            elif w in (chess.WHITE, chess.BLACK):
                winner_style = side[w]
            else:
                winner_style = DRAW
            results[game_id] = {
                "winner_style": winner_style,
                "white_style": side[chess.WHITE],
                "moves": len(g.history),
...
async def main(n_games=100, concurrency=20):
    ...
    half = n_games // 2
    sides = [
        {chess.WHITE: "quick", chess.BLACK: "thinker"} if i < half
        else {chess.WHITE: "thinker", chess.BLACK: "quick"}
        for i in range(n_games)
    ]

    print(f"=== Starting {n_games} chess games (concurrency={concurrency}) ===", flush=True)
    print(f"Balanced: {half} Quick=White, {n_games - half} Thinker=White", flush=True)
    start_t = time.time()
    ...
    elapsed = time.time() - start_t
    style_c = Counter()
    when_quick_w = Counter()
    when_thinker_w = Counter()
    ...
    print()
    print("--- Chess result codes ---")
    for k, v in result_counter.most_common():
        print(f"  {k}: {v}")
    print()
    print("--- By White assignment ---")
    print(f"Quick=White ({sum(when_quick_w.values())}): Q={when_quick_w.get('quick',0)} T={when_quick_w.get('thinker',0)} D={when_quick_w.get(DRAW,0)}")
    print(f"Thinker=White ({sum(when_thinker_w.values())}): Q={when_thinker_w.get('quick',0)} T={when_thinker_w.get('thinker',0)} D={when_thinker_w.get(DRAW,0)}")
    print()
