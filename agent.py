"""Quick (intuition) and Thinker (CoT) chess players using Gemma + JSON schema."""
...
import httpx
import chess
from chess_game import board_to_ascii


QUICK_SYSTEM = """あなたはチェスの対局者です。スタイルは「直感型・即答」。
盤面と合法手リスト（UCI 表記）を見て、深く読まずに直感で着手を決めます。

UCI 表記:
- 通常の手: 例 "e2e4"（e2 から e4 へ）
- プロモーション: 末尾に駒文字 (q=Queen, r=Rook, b=Bishop, n=Knight) 例 "e7e8q"
- キャスリング: キングが 2 マス動く形 例 "e1g1"（白のキングサイドキャスリング）
- 必ず提示された合法手リストの中から1つ選ぶこと

JSON で次を返してください:
- intent: この手で何を狙っているかの宣言文（例 "中央のポーンを突いてビショップの利きを通す狙い"）
- move: UCI 表記の手"""

THINKER_SYSTEM = """あなたはチェスの対局者です。スタイルは「熟考型・じっくり読む」。
盤面と合法手リスト（UCI 表記）を見て、最低3つの候補手を比較検討します。

検討の観点:
- キングの安全（攻撃に晒されていないか、キャスリングの要否）
- 駒の働き（中央支配、ルーク・ビショップの利き）
- 駒の価値（ポーン1, ナイト/ビショップ3, ルーク5, クイーン9 の概算）
- 先・後の応手（相手の最善応手で自分が悪くならないか）
- エンディングなら駒の昇格ルートと対向関係

UCI 表記:
- 通常の手: 例 "e2e4"、プロモーション: "e7e8q"、キャスリング: "e1g1"

JSON で次を返してください:
- thinking: 候補手3つ以上を比較した詳細な検討プロセス
- summary: 最終決定の理由を一行で端的に
- move: UCI 表記の手（必ず合法手リストから選ぶこと）"""
...
class GemmaPlayer:
    ...
    def _build_user_msg(self, board, legal_uci):
    ...
    def choose_move_streaming(self, board, legal_uci,
                              on_chunk: Callable[[str, str], None] | None = None,
                              max_retries: int = 2) -> AgentResponse:
        user_msg = self._build_user_msg(board, legal_uci)
        messages = [
